...
function jcp_niche_render_split_media_block( array $props, string $path, string $niche_key = '', array $options = [] ): void {
	...
	<div class="demo-preview-content jcp-split-layout jcp-media-text-grid <?php echo esc_attr( jcp_media_position_class( $position ) ); ?>" data-jcp-split-path="<?php echo esc_attr( $path ); ?>" data-jcp-media-position-path="<?php echo esc_attr( $path . '.media_position' ); ?>">
		<div class="demo-preview-text jcp-split-col jcp-split-col--copy" data-jcp-split-col="copy">
			<?php if ( ! empty( $props['show_badge'] ) && trim( (string) ( $props['badge'] ?? '' ) ) !== '' ) : ?>
			...
			<?php endif; ?>
			<?php if ( ! empty( $props['show_subheadline'] ) && trim( (string) ( $props['subheadline'] ?? '' ) ) !== '' ) : ?>
				<p class="rankings-subtitle jcp-split-subheadline"<?php jcp_niche_editable_attr( $path . '.subheadline' ); ?>><?php echo esc_html( (string) $props['subheadline'] ); ?></p>
			<?php endif; ?>
			<?php if ( ! empty( $props['show_cue'] ) && trim( (string) ( $props['cue'] ?? '' ) ) !== '' ) : ?>
			...
			<?php if ( ! empty( $props['show_cta'] ) || ! empty( $props['show_cta_note'] ) ) : ?>
				<div class="demo-cta-wrapper">
					...
					<?php endif; ?>
					<?php if ( ! empty( $props['show_cta_note'] ) && trim( (string) ( $props['cta_note'] ?? '' ) ) !== '' ) : ?>
						<p class="demo-cta-note"<?php jcp_niche_editable_attr( $path . '.cta_note' ); ?>><?php echo esc_html( (string) $props['cta_note'] ); ?></p>
					<?php endif; ?>
				</div>
			<?php endif; ?>
			<?php if ( ! empty( $props['show_divider'] ) ) : ?>
				<hr class="jcp-split-media-block__rule" aria-hidden="true" />
			<?php endif; ?>
		</div>
