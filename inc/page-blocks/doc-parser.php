...
function jcp_page_parse_document( string $text, string $page_key = '', string $page_label = '', string $page_kind = 'industry' ): array {
	...
	$legacy['preset']    = $page_kind === 'referral' ? 'referral' : ( $page_kind === 'industry' ? 'industry' : ( $page_kind === 'home' ? 'home' : 'marketing' ) );
 ...
 */
function jcp_page_parse_document_with_report( string $text, string $page_key = '', string $page_label = '', string $page_kind = 'industry' ): array {
	$legacy = jcp_niche_parse_document( $text, $page_key, $page_label );
	if ( $page_kind === 'referral' || ( $legacy['page_type'] ?? '' ) === 'referral' ) {
		$legacy['page_type'] = 'referral';
		$page_kind           = 'referral';
	...
	$report              = jcp_page_doc_build_import_report( $legacy, $content, $page_kind );
