<?php
/**
 * Help Articles listing content (hero, search/filter bar, grid, script).
 * Expects $args with: help_query, help_has_posts, total_posts, help_terms.
 *
 * @package JCP_Core
 */
$help_query     = isset( $args['help_query'] ) ? $args['help_query'] : null;
$help_has_posts = isset( $args['help_has_posts'] ) ? $args['help_has_posts'] : false;
$total_posts    = isset( $args['total_posts'] ) ? (int) $args['total_posts'] : 0;
$help_terms     = isset( $args['help_terms'] ) ? $args['help_terms'] : [];
if ( ! $help_query instanceof WP_Query ) {
	$help_has_posts = false;
	$total_posts    = 0;
}
?>
...
(function() {
  ...
  function updatePlaceholder(visible) {
    ...
    var num = visible >= 0 ? visible : totalCards;
    searchInput.placeholder = num === 1 ? singular : pluralTpl.replace('%d', String(num));
  }
